...
export async function callAgent(agent: any, config: any, userInput: string) {
  ...
  for await (const chunk of stream) {
    if ("agent" in chunk) {
      ...
      messagePromises.push(Promise.resolve(chunk.agent.messages[0].content));
    } else if ("tools" in chunk) {
      messagePromises.push(Promise.resolve(chunk.tools.messages[0].content));
    }
    console.log("-------------------");
  }

  const messages = await Promise.all(messagePromises);
  // const splitMessages = messages.flatMap(message => message.split('\n'));
